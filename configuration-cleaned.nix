...

{
  ...
  # Disable HDA power saving to prevent HDMI audio pops
  ...
  services.pipewire = {
    ...
    # Power saving is disabled at kernel level (boot.extraModprobeConfig) to prevent pops
    ...
    wireplumber.configPackages = [
      ...
      '')
      (pkgs.writeTextDir "share/wireplumber/wireplumber.conf.d/20-game-audio.conf" ''
        monitor.alsa.rules = [
          {
            matches = [
              { application.name = "SevenDeadlySins_Origin" }
            ]
            actions = {
              update-props = {
